...
import {
  AgentUser, createAgent, deleteAgent, listAgents, updateAgent,
} from '../api/client';
import {
  Badge, Button, Card, CardHeader, ClearFiltersButton, EmptyState,
  FilterDropdown, FilterPill, Input, Modal, PageLoader, SearchInput, PagedTable,
} from '../components/ui';
...
export default function AgentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return agents.filter((a) => {
      if (q) {
        const hay = `${a.first_name || ''} ${a.last_name || ''} ${a.email || ''} ${a.username || ''}`.toLowerCase();
  ...
  return (
    <div className='p-6 space-y-5'>
      <div className='page-header-bar'>
        ...
        {isAdmin && (
          <Button icon={<Plus className='w-3.5 h-3.5' />} onClick={() => setCreateOpen(true)}>
            New Admin
          </Button>
        )}
      </div>
      ...
      <div className='space-y-3'>
        <div className='filter-bar'>
          <SearchInput value={search} onChange={setSearch} placeholder='Search by name, email or username…' />
          <div className='flex items-center gap-2 flex-wrap'>
            <FilterDropdown
              label='Status' value={filterActive} onChange={setFilterActive} color='green'
      ...
      <Card>
        ...
        {hasActiveFilters && filtered.length === 0 ? (
          <EmptyState title='No users match your filters' description='Try adjusting or clearing the filters above.' />
        ) : (
          <PagedTable
            cols={[
              {
                ...
                render: (r: Row) => {
                  ...
                  return (
                    <div className='flex items-center gap-3'>
                      ...
                      <span className='font-semibold text-[#0F1117] truncate'>{r.first_name}</span>
                    </div>
                  );
                },
              },
              {
                ...
                render: (r: Row) => (
                  <span className='font-semibold text-[#0F1117] truncate'>{r.last_name || '—'}</span>
                ),
              },
              {
                header: 'Username', width: '15%',
                render: (r: Row) => (
                  <span className='text-sm font-mono text-[#4B5563] truncate'>{r.username || '—'}</span>
                ),
              },
              {
                header: 'Email', width: '22%',
                render: (r: Row) => (
                  <span className='text-sm text-[#6B7280] truncate'>{r.email || '—'}</span>
                ),
              },
              {
                ...
                render: (r: Row) => (
                  <Badge
                    label={r.role}
                    color={r.role === 'admin' ? 'indigo' : r.role === 'supervisor' ? 'purple' : 'gray'}
                  />
                ),
              },
              {
                ...
                render: (r: Row) => {
                  ...
                  return (
                    <div className='flex items-center gap-1.5'>
                      <button
                        onClick={() => isSelf ? setEditSelfOpen(true) : setEditUser(r)}
      ...
      {createOpen && (
        <CreateAgentModal
          ...
          onCreated={() => { setCreateOpen(false); qc.invalidateQueries({ queryKey: ['agents'] }); }}
        />
      )}
      {editUser && (
        <EditAgentModal
          ...
          onSaved={() => { setEditUser(null); qc.invalidateQueries({ queryKey: ['agents'] }); }}
        />
      )}
      {deleteUser && (
        <DeleteAgentModal
          ...
          onDeleted={() => { setDeleteUser(null); qc.invalidateQueries({ queryKey: ['agents'] }); }}
        />
      )}
      {editSelfOpen && selfRow && (
        <EditSelfModal
          ...
          onSaved={() => { setEditSelfOpen(false); qc.invalidateQueries({ queryKey: ['agents'] }); }}
        />
      )}
    </div>
  );
...
function CreateAgentModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    m.mutate({ email: email.trim(), password, first_name: firstName.trim(), last_name: lastName.trim(), username: username.trim(), role: 'admin' });
  };
    ...
    <Modal title='New Admin' open={true} onClose={onClose}>
      <form onSubmit={handleSubmit} className='space-y-3'>
        ...
        <div className='space-y-1'>
          ...
          <div className='px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-500 capitalize'>Admin</div>
        </div>
...
function EditAgentModal({ target, onClose, onSaved }: { target: AgentUser; onClose: () => void; onSaved: () => void }) {
  ...
  const m = useMutation({
    mutationFn: () => {
      const payload: any = {
        ...
        username: username.trim(), is_active: isActive,
      };
      if (wantsPasswordReset) payload.password = newPassword;
    ...
    <Modal title={`Edit ${target.first_name} ${target.last_name}`} open={true} onClose={onClose}>
      <form className='space-y-4' onSubmit={handleSubmit}>
        ...
        <div className='space-y-1'>
          ...
          <div className='px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-500'>{target.email}</div>
        </div>
        <div className='space-y-1'>
          <label className='block text-xs text-gray-500'>Role</label>
          <div className='px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-500 capitalize'>{target.role}</div>
        </div>
        ...
        <label className='flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition'>
          ...
          <span className='text-sm text-gray-700'>Active — user can sign in and be assigned to campaigns</span>
        </label>

        <div className='space-y-3 pt-1 border-t border-gray-100'>
          <p className='text-xs font-medium text-gray-500 pt-2'>
            Reset password <span className='font-normal text-gray-400'>(leave blank to keep unchanged)</span>
          </p>
          <PasswordInput label='New password (min 8 chars)' value={newPassword} onChange={setNewPassword} placeholder='Enter new password' />
          <PasswordInput label='Confirm new password' value={confirmPassword} onChange={setConfirmPassword} placeholder='Re-enter new password' />
        ...
        <div className='flex justify-end gap-2 pt-2 border-t border-gray-100'>
          ...
          <Button type='submit' loading={m.isPending}
            disabled={!firstName.trim() || !lastName.trim() || !username.trim() || (wantsPasswordReset && newPassword !== confirmPassword)}>
            Save Changes
          </Button>
        </div>
      </form>
...
// ─── Edit Self Modal ───────────────────────────────────────────────────────────
// (unchanged — self-edit does not expose username to keep it consistent)
// If you want self-edit to also allow username change, add the same pattern here.

function EditSelfModal({ target, onClose, onSaved }: { target: AgentUser; onClose: () => void; onSaved: () => void }) {
  const [firstName, setFirstName] = useState(target.first_name || '');
  const [lastName, setLastName] = useState(target.last_name || '');
  ...
  const m = useMutation({
    mutationFn: () => {
      const payload: any = { first_name: firstName.trim(), last_name: lastName.trim(), email: email.trim(), username: username.trim() };
      if (wantsPasswordChange) { payload.current_password = currentPassword; payload.password = newPassword; }
      return updateAgent(target.id, payload);
    },
    onSuccess: () => onSaved(),
    ...
    <Modal title='Edit my profile' open={true} onClose={onClose}>
      <form onSubmit={handleSubmit} className='space-y-4'>
        <div className='flex items-center gap-3 pb-3 border-b border-gray-100'>
          <div className='w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold text-white bg-indigo-500 flex-shrink-0'>{initials}</div>
          <div>
            <div className='text-sm font-semibold text-gray-800'>{target.first_name} {target.last_name}</div>
        ...
        <div className='space-y-3 pt-1 border-t border-gray-100'>
          ...
          <PasswordInput label='Current password' value={currentPassword} onChange={setCurrentPassword} placeholder='Enter current password' />
          <PasswordInput label='New password (min 8 chars)' value={newPassword} onChange={setNewPassword} placeholder='Enter new password' />
          <PasswordInput label='Confirm new password' value={confirmNewPassword} onChange={setConfirmNewPassword} placeholder='Re-enter new password' />
        ...
        <div className='flex justify-end gap-2 pt-2 border-t border-gray-100'>
          <Button variant='secondary' onClick={onClose} type='button'>Cancel</Button>
          <Button type='submit' loading={m.isPending}
            disabled={!firstName.trim() || !lastName.trim() || !username.trim() || !email.trim() || (wantsPasswordChange && newPassword !== confirmNewPassword)}>
            Save Changes
          </Button>
        </div>
      </form>
...
function DeleteAgentModal({ target, onClose, onDeleted }: { target: AgentUser; onClose: () => void; onDeleted: () => void }) {
  const [error, setError] = useState('');
  const m = useMutation({
    mutationFn: () => deleteAgent(target.id),
    onSuccess: () => onDeleted(),
    onError: (e: any) => setError(e?.response?.data?.error || 'Failed to delete user'),
  });
  return (
    <Modal title={`Delete ${target.first_name} ${target.last_name}?`} open={true} onClose={onClose}>
      <div className='space-y-4'>
        <p className='text-sm text-gray-600'>
          This removes <strong>{target.email}</strong> from your organization. If the user has historical activity, the account is deactivated instead of hard-deleted so audit history is preserved.
        </p>
        {error && <p className='text-xs text-red-500'>{error}</p>}
