...
const listExtensions = (): Promise<{ data: ExtensionOption[] }> =>
  api.get('/users/extensions').then((r) => r.data);
...
type FormState = {
  ...
  role: 'agent' | 'supervisor'; reporting_to: string; sip_extension: string;
};
...
// ─── Reporting To Select (admin + supervisor only) ───────────────────────────
function ReportingToSelect({ options, value, onChange, loading }: {
  ...
  // Only show admin and supervisor
  const filtered = useMemo(() => {
    const adminAndSupervisors = options.filter((o) => o.role === 'admin' || o.role === 'supervisor');
    const q = search.trim().toLowerCase();
    if (!q) return adminAndSupervisors;
    return adminAndSupervisors.filter((o) =>
...
function ExtensionSelect({ options, value, onChange, loading, required }: {
  options: ExtensionOption[]; value: string; onChange: (v: string) => void;
  ...
  const filtered = useMemo(() => {
    ...
    return options.filter((o) =>
      ...
      String(o.effetive_number ?? '').includes(q),
    );
  }, [options, search]);
          ...
          <div className="max-h-36 overflow-y-auto bg-white">
            ...
            {filtered.length === 0 ? (
              ...
              <div
                ...
                className={`px-3 py-2 cursor-pointer hover:bg-orange-50 flex items-center justify-between gap-2 ${value === opt.extension_id ? 'bg-orange-50' : ''}`}
              >
                <div>
                ...
                {opt.effetive_number && (
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold shrink-0 bg-blue-50 text-blue-600">
                    {opt.effetive_number}
                  </span>
                )}
              </div>
...
export default function UsersPage() {
  ...
  const [editForm, setEditForm] = useState<Partial<FormState & { is_active: boolean; sip_password: string }>>({});
  const [search, setSearch] = useState('');
          ...
          <button
            onClick={(e) => {
              ...
              setEditForm({
                ...
                role: u.role, is_active: u.is_active, reporting_to: u.reporting_to ?? '',
                sip_extension: u.sip_extension ?? '', sip_password: u.sip_password ?? '',
              });
            }}
  ...
  // Create form validation: sip_extension is now mandatory
  const createIsValid = !!(form.first_name && form.last_name && form.email && form.password && form.sip_extension);
  ...
  return (
    <div className="p-6 md:p-8 w-full space-y-6 animate-fade-up">
      ...
      {/* ── Create Modal ── */}
      {/*
        Width increased via maxWidth override; scrollable body so the modal
        stays compact even on small screens.
      */}
      <Modal
  title="Add New User"
  open={createOpen}
...
>
        ...
        <div className="overflow-y-auto max-h-[65vh] pr-1 space-y-4">
          ...
          {/* Reporting To — admin + supervisor only */}
          <ReportingToSelect
          ...
          {/* SIP Extension — mandatory, dropdown */}
        ...
        <div className="flex justify-end gap-2 pt-3 border-t border-gray-100 mt-3">
          ...
          <Button
            onClick={() => createMut.mutate({
              ...
              sip_password: null,  // removed from create form
            })}
