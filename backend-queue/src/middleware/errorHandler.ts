...
export function errorHandler(
  ...
  // statement_timeout in queue pool — surface as 503 so client retries
  if ((err as any).code === '57014') {
    res.status(503).json({ error: 'Query timed out', detail: 'queue pool statement_timeout exceeded' });
    return;
  }
